...
- **Install:** `npx expo install @sentry/react-native` — **never `npm install`** (the build-16 lesson). **Build-time correction (2026-06-12):** the pre-build verification claimed v8.x is required, but Expo SDK 55's authoritative `bundledNativeModules` pins **`~7.11.0`** (its `react-native` peer is `>=0.65.0`, covering RN 0.83), and that's what `expo install` resolves and what Expo validated against this SDK. We use **7.11.0** — trust `expo install` / `expo-doctor` over the web-search synthesis.
- **`promise` dependency (build-breaker, found 2026-06-13):** the first EAS build failed in the "Bundle JavaScript" phase — `Unable to resolve module promise/setimmediate/done` from `@sentry/react-native`. The Sentry SDK deep-imports `promise/setimmediate/done`, but `promise` is only **nested under `react-native`**, not hoisted, so Metro can't resolve it from the Sentry package. **Fix: add `promise` as a direct dep** (`npx expo install promise` → `^8.3.0`, matching RN's nested copy). This is **version-independent** (v7 and v8 both import it — confirmed the v8 "fix" was a red herring; reverted to bundled 7.11.0). `expo-doctor` + the mocked jest suite don't catch it because neither runs the real Metro bundle; verify with `npx expo export --platform ios` before any `eas build`.
...
- **Capture, tuned post-B1:** native crashes automatic; JS errors automatic; in `client.ts`, `Sentry.captureException` only for **unexpected network errors + 5xx** `ApiError`s; **all 4xx (incl. 401) and offline `Network request failed` → breadcrumbs, not events** (expected/survivable states — must not page, and offline must not burst the shared quota). Query strings are stripped from tags/breadcrumbs (no search terms/IDs). `tracesSampleRate: 0` (errors only).
- **Build safety:** the `@sentry/react-native/expo` config plugin injects Xcode source-map/dSYM **upload** phases that hard-fail the build if `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` are absent. Until the operator wires those, `eas.json` sets `SENTRY_DISABLE_AUTO_UPLOAD: "true"` on all build profiles so builds succeed (no symbolication upload). Operator removes it once the token/org/project EAS env are set.
